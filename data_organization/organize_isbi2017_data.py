...
# Prepare filenames
training_dataset_name = "ISIC-2017_Training_Data.zip"
training_dataset_labels_name = "ISIC-2017_Training_Part3_GroundTruth.csv"
...
if not os.path.exists(training_data_folder):
    ...
    os.makedirs(os.path.join(training_data_folder, 'seborrheic_keratosis'))
    os.makedirs(os.path.join(training_data_folder, 'melanoma'))
...
if not os.path.exists(validation_data_folder):
    ...
    os.makedirs(os.path.join(validation_data_folder, 'seborrheic_keratosis'))
    os.makedirs(os.path.join(training_data_folder, 'melanoma'))
...
if not os.path.exists(training_dataset_full_filename):
    ...
    with open(training_dataset_full_filename, newline='\n') as csvfile:
        ...
        for row in my_file_reader:
            ...
            # Determine the class corresponding to current entry
            if row[1]==0 and row[2]==0:
                current_class = 'bening'
            elif row[1]==1 and row[2]==0:
                current_class = 'seborrheic_keratosis'
            else
                current_class = 'melanoma'
            ...
            if images_counter <= num_training_images:
                ...
                os.rename(os.path.join(tmp_folder, training_dataset_name, current_image_filename), os.path.join(training_data_folder, current_class, current_image_filename))
            else:
                # cut and paste each file to the corresponding label folder on the validation data
                os.rename(os.path.join(tmp_folder, training_dataset_name, current_image_filename), os.path.join(validation_data_folder, current_class, current_image_filename))
